...
{
    public async Task LiveTableScreen1(List<Tuple<string, int, int>> seats)
    {
    ...
    public async Task LiveTableScreen2(List<Tuple<string, int, int>> seats)
    {
        var table = new Table().LeftAligned().BorderColor(Color.Fuchsia);
        var delayTable = 0;
        var delaySeats = 0;
        await AnsiConsole.Live(table).AutoClear(false).StartAsync(async ctx =>
        {
